...
/*
 * Top band already has a strong site — point them at keeping it that way. Every
 * other band gets a call to talk the results through. The booking link leaves
 * the site, so it opens in a new tab: the results only exist in this tab, and
...
const ctaFor = (band, cfg) =>
  band.cls === 'excellent'
    ...
    : {
        ...
        text: "Book a free call and we'll walk through your scorecard together — what's costing you credibility, and what to fix first.",
        label: 'Book a call',
        url: cfg.reviewUrl,
        external: true,
      };
...
const resultsView = ({ state, cfg }) => {
  ...
  <div class="sc-card">
    ...
    <div class="sc-cta">
      ...
      <a class="stature-btn stature-btn--md stature-btn--primary is-on-navy" href="${escapeHtml(cta.url)}"${cta.external ? ' target="_blank" rel="noopener"' : ''}>${cta.label}</a>
    </div>
...
const initScorecard = (root) => {
  ...
  const ctx = {
    ...
    cfg: {
      ...
      reviewUrl: root.dataset.reviewUrl || '#',
